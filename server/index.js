...
const serverTick = 40;
app.get('/', (_, res) => res.sendFile('client/dist/index.html'));
...
let spawns = [];
let size;
const roundTimeMax = 90;
let roundTime = 0;
let currentTime = 0;
let highscore = { name: 'muda', score: 0 };
const key = 'ZeroTixMe'; //STOP LOOKING AT THE CODE >:C
/*(async () => {
...
function changeMap(number) {
  ...
  const grid = require(`./maps/map${number}.json`);
  ...
  for (const row in grid) {
    for (const col in grid[row]) {
      if ('' + grid[row][col] === 'rgb(0, 0, 255)') {
        ...
        spawns.push({ row, col });
        spawns.push({ row, col });
      } else if ('' + grid[row][col] === 'rgb(255, 0, 0)' || '' + grid[row][col] === 'rgb(0, 0, 0)') {
...
function updateGameState(clients, players) {
  ...
  lastTime = Date.now();
  currentTime += delta;
  const old_roundTime = roundTime;
  roundTime += delta;
  const old_highscore = highscore;
  ...
  const pack = Player.pack({ players, arena, platforms, currentTime });
  const arrowPackages = Arrow.pack({
    arrows,
    removePack,
    platforms,
    currentTime,
  ...
  const arrowPack = arrowPackages.pack;
  ...
  for (const i of Object.keys(clients)) {
    ...
    if (clientSocket.readyState === WebSocket.OPEN) {
      ...
      clientSocket.send(msgpack.encode(updateObject));
  ...
  ws.on('message', (msg) => {
    try {
      ...
      if (data.type === 'join') {
        ...
        if (data.value.slice(0, 5).toLowerCase() === '/name' || data.value.slice(0, 5).toLowerCase() === '/nick') {
          ...
          const name = array[1];
          const message = array.slice(2).join(' ');
          console.log(message);
          for (const i of Object.keys(players)) {
          ...
          if (array.length >= 2) {
            ...
            const iter = parseInt(array[1]) ? parseInt(array[1]) : 10;
