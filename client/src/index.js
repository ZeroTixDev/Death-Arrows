...
const arrows = Object.create(null);
const keys = new Array(8).fill(false);
...
let serverTick;
let lbPlayers = [];
images.winter.src = require('./assets/winter-wreath.png');
images.spring.src = require('./assets/spring-wreath.png');
images.santa.src = require('./assets/santa-hat.png');
let arena = null;
let scale = 0;
let selfId = null;
let time = 0;
...
class Arrow {
    ...
    interpolate(delta) {
        if (delta <= 1 / serverTick && config.interp) {
            ...
            this.states[0].x = lerp(this.states[0].x, this.serverState.x, delta * serverTick);
            this.states[0].y = lerp(this.states[0].y, this.serverState.y, delta * serverTick);
    ...
    draw(delta) {
        ...
        if (
            x + this.width < -detectPadding ||
            x > canvas.width + detectPadding ||
            y > canvas.height + detectPadding ||
            y + this.height < -detectPadding
        ) {
            return;
        }
        arrowDraw++;
...
class Particle {
    ...
    draw() {
        ...
        if (
            x + this.size / 2 + this.size < -detectPadding ||
            x + this.size / 2 > canvas.width + detectPadding ||
            y + this.size / 2 > canvas.height + detectPadding ||
            y + this.size + this.size / 2 < -detectPadding
        ) {
            return;
        }
        particleDraw++;
...
class Player {
    ...
    interpolate(delta) {
        if (delta <= 1 / serverTick && config.interp) {
            ...
            this.states[0].x = lerp(this.states[0].x, this.serverState.x, delta * serverTick);
            this.states[0].y = lerp(this.states[0].y, this.serverState.y, delta * serverTick);
            this.rotStates[0].rot = lerp(this.rotStates[0].rot, this.serverState.rot, delta * serverTick);
        } else {
    ...
    draw(delta) {
        ...
        if (
            x + this.radius < -detectPadding ||
            x - this.radius > canvas.width + detectPadding ||
            y - this.radius > canvas.height + detectPadding ||
            y + this.radius < -detectPadding
        ) {
            return;
        }
        playerDraw++;
      ...
      particles.push(
        ...
        if (this.hitWall && !this.hitLock && config.particles) {
            for (let i = 0; i < 15; i++) {
                particles.push(
                    new Particle(
                        (this.states[1] !== undefined ? this.states[1].x : this.states[0].x) +
                            Math.random() * 25 -
                            12.5,
                        (this.states[1] !== undefined ? this.states[1].y : this.states[0].y) +
                            Math.random() * 25 -
                            12.5,
                        Math.random() * 3 + 1,
                        platformColor,
        ...
        if (this.chatTime > 0) {
            ...
            ctx.fillRect(Math.round(x - width / 2 - 3), Math.round(y - 63), Math.round((width * 2) / 2 + 6), 30);
...
ws.addEventListener('message', (datas) => {
    ...
    if (msg.type === 'init') {
            ...
            if (arrow) {
                if (type === 'player' && config.particles) {
                    for (let i = 0; i < 500; i++) {
                        ...
                        particles.push(
                            new Particle(
                                arrow.states[1] !== undefined ? arrow.states[1].x : arrow.states[0].x,
                                arrow.states[1] !== undefined ? arrow.states[1].y : arrow.states[0].y,
                                Math.random() * 4 + 1,
                                color,
                                {
        ...
        ctx.fillStyle = 'white';
        ctx.fillText('You have been kicked by an Administrator.', canvas.width / 2, canvas.height / 2);
...
class Star {
    ...
    draw(delta) {
        ...
        this.radius = this.radius <= 0 ? 0.1 : this.radius >= 4 ? Math.random() * 1.2 : this.radius;
        this.iter += delta;
...
function render(time) {
    ...
    if (nameSpan.innerText !== players[selfId].username) nameSpan.innerText = players[selfId].username;
    ...
    for (let i of Object.keys(lbPlayers)) {
        ...
        if (lbPlayers[i].type !== 'YouLol') {
            ...
            ctx.fillText(p + '. ' + lbPlayers[i].name + ': ' + lbPlayers[i].kills, 1290, 150 + i * 50);
        } else {
            ctx.fillStyle = 'white';
            ctx.fillText(index + '. ' + lbPlayers[i].name + ': ' + lbPlayers[i].kills, 1290, 150 + i * 50);
        }
        ctx.textAlign = 'center';
    }
    ctx.font = '25px Verdana, Geneva, sans-serif';
    ...
    ctx.fillRect(canvas.width / 2 - 100, canvas.height, 200, -50 * (superCooldown.current / superCooldown.max));
    ctx.fillStyle = 'white';
    ctx.strokeStyle = 'black';
    ctx.lineWidth = 1;
    ctx.beginPath();
